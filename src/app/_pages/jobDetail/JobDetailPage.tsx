...
import { formatDate } from "@jobquest/utils";
...
interface JobDetailParams extends Record<string, string> {
  job_id: string;
}

const JobDetailPage = ({ jobs }: JobDetailProps) => {
  const { job_id } = useParams<JobDetailParams>();
  ...
  if (jobs.length === 0)
    return (
      <div className="max-w-6xl pt-32 w-11/12 mx-auto">
        <Spinner classNames="w-10 h-10"></Spinner>
      </div>
    );
  if (!job) {
    return (
    ...
    <section className="max-w-6xl pt-28 w-11/12 mx-auto">
      <div className="mb-5 flex items-center">
        <div onClick={onBackHandler} className="flex gap-3 cursor-pointer">
          <BsArrowLeft className="text-3xl transition ease-linear duration-300 delay-150 hover:-translate-x-2 text-blue-700 "></BsArrowLeft>
      ...
      <div className="grid lg:grid-cols-3 gap-7 pb-10">
        <div className="bg-white p-4 lg:p-10 shadow-lg rounded-xl lg:col-span-2 grid grid-cols-1 gap-7 place-self-start w-full">
          <div className="flex justify-between items-center">
            ...
            <div className="hidden sm:block place-self-end">
              ...
              <div>
                <button className="bg-sky-900 w-full shadow-lg rounded-md py-2 mt-4 text-white capitalize text-lg hover:shadow-gray-400 transition ease-in-out delay-150 duration-300 hover:bg-sky-950">
                  apply now
                </button>
              </div>
            </div>
          ...
          <div className="sm:hidden">
            ...
            <div>
              <button className="bg-sky-900 w-full shadow-lg rounded-md py-2 mt-4 text-white capitalize text-lg hover:shadow-gray-400 transition ease-in-out delay-75 duration-200 hover:bg-sky-950">
                apply now
              </button>
            </div>
          </div>
        ...
        <div className="lg:sticky lg:top-20 grid place-self-start w-full">
          <div className="bg-white rounded-lg shadow-lg grid grid-cols-1 gap-4 shadow-purple-200 p-4 lg:p-10">
            ...
            <div>
              <button className="w-full bg-sky-900 py-3 text-white capitalize text-xl shadow-lg hover:shadow-gray-400 transition ease-in-out delay-100 duration-200 hover:bg-sky-950 rounded-md ">
                apply now
              </button>
            </div>
          </div>
