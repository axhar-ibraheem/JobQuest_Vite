    ...
    <article className="max-w-96 rounded-lg p-4 lg:p-10 bg-white grid shadow-lg shadow-purple-200 transition ease-in-out duration-500">
      ...
      <div className="grid sm:grid-cols-2 gap-3 mt-7">
        <div className="flex items-center gap-3 text-gray-500">
          <ImLocation className="text-2xl lg:text-3xl text-rose-900"></ImLocation>
          <p className="capitalize tracking-wide lg:text-xl">{location}</p>
        </div>

        <div className="flex items-center gap-3 text-gray-500">
          <BiCalendar className="text-2xl text-rose-900"></BiCalendar>
          <p>{formattedDate}</p>
        </div>
      </div>
      <div className="grid gap-3 sm:grid-cols-2 mt-7">
        <div className="flex items-center gap-3 text-gray-500">
          <FaToolbox className="text-2xl lg:text-3xl text-rose-900"></FaToolbox>
          <p className="capitalize tracking-wide lg:text-xl">{category}</p>
        </div>

        <div className="flex items-center gap-3">
          <BiSolidTimeFive className="text-2xl lg:text-3xl text-rose-900"></BiSolidTimeFive>
      ...
      <div className="mt-7 flex gap-3">
        <Link to={`${ROUTES.DASHBOARD}/${id}`}>
          <button className="bg-blue-800 px-4 py-2 rounded-md text-white shadow-lg hover:shadow-gray-400 capitalize tracking-wide transition ease-linear duration-300 delay-150 ">
            veiw details
          </button>
        </Link>
      </div>
